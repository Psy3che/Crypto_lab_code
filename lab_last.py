...
#     main()
# vigenere encryption uses the alphabet using the matrix drawn for all the alphabets

def viginere_enc(message, key):
    cipher = []
    for i in range(0, len(message)):
        cipher.append((message[i] + key[i % len(key)]) % 26)  # everything mod 26 for an
    return cipher

def viginere_dec(cipher, key):
    message = []
    for i in range(0, len(cipher)):
        message.append((cipher[i] - key[i % len(key)]) % 26)
    return message

def main():
    message = [0,15,15,11,4]
    key = [3,14,6]
    result_enc = viginere_enc(message, key)
    result_dec = viginere_dec(result_enc, key)
    print(result_enc)
    print(result_dec)
